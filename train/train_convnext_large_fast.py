...
策略：只训练5-10个epoch，使用更强的学习率和优化策略
预计时间：1-2小时
"""
import os
import argparse
...
def main():
    parser = argparse.ArgumentParser(description='ConvNeXt Large快速微调（目标95+分，1-2小时）')
    ...
    parser.add_argument('--epochs', type=int, default=8,
                       help='训练轮数（快速微调建议5-10轮）')
    parser.add_argument('--lr', type=float, default=0.0005,
                       help='初始学习率（快速微调可以使用稍大的学习率）')
    ...
    parser.add_argument('--save_dir', type=str, default='checkpoints_convnext_large_fast')
    ...
    # 只微调分类头，冻结backbone的前几层（加快训练）
    print("\n冻结backbone的部分层（加快训练）...")
    for name, param in model.named_parameters():
        if 'base_model' in name and 'features.0' in name or 'features.1' in name:
            param.requires_grad = False
    
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"冻结后可训练参数量: {trainable_params / 1e6:.2f}M")
    ...
    # 使用warmup + cosine退火（快速收敛）
    from torch.optim.lr_scheduler import LambdaLR
    def lr_lambda(epoch):
        if epoch < 2:  # warmup
            return (epoch + 1) / 2
        else:
            return 0.5 * (1 + np.cos(np.pi * (epoch - 2) / (args.epochs - 2)))
