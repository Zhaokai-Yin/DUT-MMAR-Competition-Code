...
class ConvNeXtLargeExtractor(nn.Module):
    """ConvNeXt Large特征提取器（与92.54分完全一致）"""
    def __init__(self):
...
class MultiModalConvNeXtPoseExtractor(nn.Module):
    """多模态ConvNeXt Large + 骨骼点特征提取器（与92.54分完全一致）"""
...
def main():
    parser = argparse.ArgumentParser(description='基于92.54分的最终优化版本（目标95+分）')
    ...
    parser.add_argument('--output', type=str, default='submission_95plus_final.csv')
    ...
    if args.classifier == 'ensemble':
        ...
        clf_svm = Pipeline([
            ...
            ('clf', SVC(
                kernel='rbf', probability=True, C=3.5, gamma='scale',  # 从3.0微调到3.5
        ...
        # 2. Logistic Regression（微调C值）
        print("  训练 Logistic Regression...")
        clf_lr = Pipeline([
            ('scaler', StandardScaler()),
            ('clf', LogisticRegression(
                max_iter=5000, solver='lbfgs', C=2.2, n_jobs=-1, random_state=42  # 从2.0微调到2.2
            ))
        ])
        clf_lr.fit(train_features, train_labels)
        print("  ✓ Logistic Regression 完成")
        
        # 3. Random Forest（微调参数）
        print("  训练 Random Forest...")
        clf_rf = Pipeline([
            ('scaler', StandardScaler()),
            ('clf', RandomForestClassifier(
                n_estimators=550, max_depth=22, n_jobs=-1, random_state=42  # 从500/20微调到550/22
            ))
        ])
        ...
        classifier = Pipeline([
            ('scaler', StandardScaler()),
            ('clf', SVC(
                kernel='rbf', probability=True, C=3.5, gamma='scale',
                decision_function_shape='ovr', max_iter=10000, random_state=42
            ))
        ])
        print("开始训练SVM分类器...")
        ...
        classifier = Pipeline([
            ('scaler', StandardScaler()),
            ('clf', LogisticRegression(
                max_iter=5000, solver='lbfgs', C=2.2, n_jobs=-1, random_state=42
            ))
        ])
        ...
        classifier = Pipeline([
            ('scaler', StandardScaler()),
            ('clf', RandomForestClassifier(
                n_estimators=550, max_depth=22, n_jobs=-1, random_state=42
            ))
        ])
    ...
    # 后处理：温度缩放（微调）
    predictions_proba = predictions_proba ** 0.93  # 从0.95微调到0.93
