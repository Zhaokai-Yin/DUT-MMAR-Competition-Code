"""
基于92.04分的增强版本
整合更强的backbone + 骨骼点检测
目标：95分+
"""
import os
import argparse
import torch
import torch.nn as nn
import torch.nn.functional as F
import numpy as np
import pandas as pd
from torch.utils.data import DataLoader
from torchvision import transforms
from tqdm import tqdm
from sklearn.linear_model import LogisticRegression
from sklearn.svm import SVC
from sklearn.ensemble import RandomForestClassifier, GradientBoostingClassifier
from sklearn.preprocessing import StandardScaler, RobustScaler
from sklearn.pipeline import Pipeline
from sklearn.model_selection import cross_val_score, StratifiedKFold, GridSearchCV
from sklearn.decomposition import PCA
from sklearn.feature_selection import SelectKBest, f_classif
import joblib
import warnings
# 过滤所有警告，包括FutureWarning
warnings.filterwarnings('ignore', category=FutureWarning)
warnings.filterwarnings('ignore', category=UserWarning)
warnings.filterwarnings('ignore')

from dataset import MultiModalDataset
import torchvision.models as models

# 尝试导入MediaPipe（可选）
try:
    import mediapipe as mp
    MEDIAPIPE_AVAILABLE = True
except ImportError:
    MEDIAPIPE_AVAILABLE = False
    print("警告: MediaPipe未安装，将跳过骨骼点检测。安装命令: pip install mediapipe")


class StrongBackboneExtractor(nn.Module):
    """更强的backbone特征提取器"""
    def __init__(self, backbone_name='convnext_large'):
        super(StrongBackboneExtractor, self).__init__()
        self.backbone_name = backbone_name
        
        if backbone_name == 'convnext_large':
            try:
                self.model = models.convnext_v2_large(pretrained=True)
                self.feature_dim = 1536
            except:
                self.model = models.convnext_large(pretrained=True)
                self.feature_dim = 1536
        elif backbone_name == 'regnet':
            # RegNet Y-32GF (最强)
            try:
                self.model = models.regnet_y_32gf(pretrained=True)
                self.feature_dim = 3712
            except:
                # 降级到Y-16GF
                try:
                    self.model = models.regnet_y_16gf(pretrained=True)
                    self.feature_dim = 3024
                except:
                    # 降级到Y-8GF
                    self.model = models.regnet_y_8gf(pretrained=True)
                    self.feature_dim = 2016
        elif backbone_name == 'efficientnet_v2':
            # EfficientNet V2 Large
            try:
                self.model = models.efficientnet_v2_l(pretrained=True)
                self.feature_dim = 1280
            except:
                self.model = models.efficientnet_v2_m(pretrained=True)
                self.feature_dim = 1280
        elif backbone_name == 'swin':
            # Swin Transformer Large
            try:
                from torchvision.models import swin_v2_b, Swin_V2_B_Weights
                self.model = swin_v2_b(weights=Swin_V2_B_Weights.IMAGENET1K_V1)
                self.feature_dim = 1024
            except:
                try:
                    from torchvision.models import swin_b, Swin_B_Weights
                    self.model = swin_b(weights=Swin_B_Weights.IMAGENET1K_V1)
                    self.feature_dim = 1024
                except:
                    print("警告: Swin Transformer不可用，使用ConvNeXt Large")
                    self.model = models.convnext_large(pretrained=True)
                    self.feature_dim = 1536
        else:
            # 默认使用ConvNeXt Large
            self.model = models.convnext_large(pretrained=True)
            self.feature_dim = 1536
        
        # 移除分类头
        if hasattr(self.model, 'classifier'):
            self.model.classifier = nn.Identity()
        elif hasattr(self.model, 'fc'):
            self.model.fc = nn.Identity()
        elif hasattr(self.model, 'head'):
            self.model.head = nn.Identity()
        
        self.model.eval()
    
    def forward(self, x):
        # x: [B, C, T, H, W]
        B, C, T, H, W = x.size()
        # 重塑为 [B*T, C, H, W]
        x = x.permute(0, 2, 1, 3, 4).contiguous()
        x = x.view(B * T, C, H, W)
        
        with torch.no_grad():
            features = self.model(x)
            # 重塑回 [B, T, feature_dim]
            if len(features.shape) == 2:
                features = features.view(B, T, -1)
            else:
                # 处理全局池化后的特征
                features = features.view(B, T, -1)
            # 时序平均池化
            features = features.mean(dim=1)
        
        return features


class PoseExtractor:
    """骨骼点特征提取器（使用MediaPipe）"""
    def __init__(self):
        if not MEDIAPIPE_AVAILABLE:
            self.mp_pose = None
            self.pose = None
            self.feature_dim = 0
            return
        
        self.mp_pose = mp.solutions.pose
        self.pose = self.mp_pose.Pose(
            static_image_mode=False,
            model_complexity=2,  # 使用高精度模型
            enable_segmentation=False,
            min_detection_confidence=0.5,
            min_tracking_confidence=0.5
        )
        # MediaPipe Pose有33个关键点，每个点有(x, y, z, visibility)4个值
        # 我们提取所有关键点的坐标和可见性
        self.feature_dim = 33 * 4  # 132维
    
    def extract_pose_features(self, images):
        """
        从图像序列中提取骨骼点特征
        Args:
            images: numpy array [T, H, W, C] 或 [B, T, H, W, C]，uint8格式，RGB通道
        Returns:
            features: numpy array [B, feature_dim] 或 [feature_dim]
        """
        if not MEDIAPIPE_AVAILABLE or self.pose is None:
            # 返回零特征
            if len(images.shape) == 4:  # [T, H, W, C]
                return np.zeros(self.feature_dim)
            else:  # [B, T, H, W, C]
                return np.zeros((images.shape[0], self.feature_dim))
        
        # 处理输入格式
        if len(images.shape) == 5:  # [B, T, H, W, C]
            B, T = images.shape[0], images.shape[1]
            all_features = []
            for b in range(B):
                video_features = []
                for t in range(T):
                    img = images[b, t].copy()  # 复制避免修改原数组
                    # 确保是uint8格式
                    if img.dtype != np.uint8:
                        if img.max() <= 1.0:
                            img = (img * 255).astype(np.uint8)
                        else:
                            img = img.astype(np.uint8)
                    # 确保是RGB格式（MediaPipe需要RGB）
                    if len(img.shape) == 3 and img.shape[2] == 3:
                        # 已经是RGB格式（从transforms得到）
                        img_rgb = img
                    else:
                        img_rgb = img
                    pose_features = self._extract_single_frame(img_rgb)
                    video_features.append(pose_features)
                # 时序平均
                video_feat = np.mean(video_features, axis=0)
                all_features.append(video_feat)
            return np.array(all_features)
        elif len(images.shape) == 4:  # [T, H, W, C]
            video_features = []
            for t in range(images.shape[0]):
                img = images[t].copy()
                if img.dtype != np.uint8:
                    if img.max() <= 1.0:
                        img = (img * 255).astype(np.uint8)
                    else:
                        img = img.astype(np.uint8)
                if len(img.shape) == 3 and img.shape[2] == 3:
                    img_rgb = img
                else:
                    img_rgb = img
                pose_features = self._extract_single_frame(img_rgb)
                video_features.append(pose_features)
            return np.mean(video_features, axis=0)
        else:
            # 单帧
            img = images.copy()
            if img.dtype != np.uint8:
                if img.max() <= 1.0:
                    img = (img * 255).astype(np.uint8)
                else:
                    img = img.astype(np.uint8)
            if len(img.shape) == 3 and img.shape[2] == 3:
                img_rgb = img
            else:
                img_rgb = img
            return self._extract_single_frame(img_rgb)
    
    def _extract_single_frame(self, img):
        """从单帧提取骨骼点特征"""
        try:
            results = self.pose.process(img)
            if results.pose_landmarks:
                landmarks = results.pose_landmarks.landmark
                # 提取所有关键点的x, y, z, visibility
                features = []
                for landmark in landmarks:
                    features.extend([landmark.x, landmark.y, landmark.z, landmark.visibility])
                return np.array(features)
            else:
                # 没有检测到骨骼点，返回零向量
                return np.zeros(self.feature_dim)
        except Exception as e:
            print(f"警告: 骨骼点提取失败: {e}")
            return np.zeros(self.feature_dim)
    
    def __del__(self):
        if self.pose is not None:
            self.pose.close()


class MultiBackbonePoseExtractor(nn.Module):
    """多backbone + 骨骼点特征提取器"""
    def __init__(self, backbone_names=['convnext_large', 'regnet', 'efficientnet_v2'], use_pose=True):
        super(MultiBackbonePoseExtractor, self).__init__()
        self.backbone_names = backbone_names
        self.use_pose = use_pose and MEDIAPIPE_AVAILABLE
        
        # 创建多个backbone提取器
        self.backbone_extractors = nn.ModuleDict()
        backbone_dims = []
        for name in backbone_names:
            extractor = StrongBackboneExtractor(name)
            self.backbone_extractors[name] = extractor
            backbone_dims.append(extractor.feature_dim)
        
        # 骨骼点提取器（非PyTorch模块）
        if self.use_pose:
            self.pose_extractor = PoseExtractor()
            pose_dim = self.pose_extractor.feature_dim
        else:
            self.pose_extractor = None
            pose_dim = 0
        
        # 计算总特征维度
        # 每个backbone对RGB、Depth、IR三个模态提取特征
        backbone_total_dim = sum(backbone_dims) * 3  # RGB + Depth + IR
        self.feature_dim = backbone_total_dim + pose_dim
    
    def forward(self, rgb_input, depth_input, ir_input):
        """
        提取多backbone特征
        注意：骨骼点特征需要在CPU上单独提取
        """
        all_features = []
        
        # 提取每个backbone的特征
        for name in self.backbone_names:
            extractor = self.backbone_extractors[name]
            rgb_feat = extractor(rgb_input)
            depth_feat = extractor(depth_input)
            ir_feat = extractor(ir_input)
            
            # 对每个backbone的特征进行L2归一化（重要！）
            # 这样可以避免不同backbone特征尺度差异导致的问题
            rgb_feat = F.normalize(rgb_feat, p=2, dim=1)
            depth_feat = F.normalize(depth_feat, p=2, dim=1)
            ir_feat = F.normalize(ir_feat, p=2, dim=1)
            
            # 拼接该backbone的三个模态特征
            backbone_feat = torch.cat([rgb_feat, depth_feat, ir_feat], dim=1)
            all_features.append(backbone_feat)
        
        # 拼接所有backbone特征
        combined_feat = torch.cat(all_features, dim=1)
        return combined_feat


def extract_features_with_tta_and_pose(model, pose_extractor, dataloader, device, 
                                      is_test_mode=False, tta_times=1, use_pose=True, 
                                      checkpoint_path=None, checkpoint_interval=50):
    """提取特征，支持TTA和骨骼点"""
    model.eval()
    all_features = []
    all_labels = []
    all_video_ids = []
    
    import time
    total_batches = len(dataloader)
    start_time = time.time()
    
    print(f"\n开始提取特征，总共 {total_batches} 个batch...")
    if use_pose and pose_extractor is not None:
        print("骨骼点检测: 已开启")
    if tta_times > 1:
        print(f"TTA: {tta_times} 次")
    print("=" * 60)
    import sys
    sys.stdout.flush()  # 确保输出立即显示
    
    with torch.no_grad():
        # 不使用tqdm，改用简单的print，确保能看到进度
        print("\n开始处理batch...")
        sys.stdout.flush()
        
        for batch_idx, batch in enumerate(dataloader):
            batch_start = time.time()
            
            # 每10个batch或第一个batch打印一次
            if batch_idx == 0 or (batch_idx + 1) % 10 == 0:
                print(f"\n[Batch {batch_idx+1}/{total_batches}] 开始处理...")
                sys.stdout.flush()
            
            if isinstance(batch, (tuple, list)) and len(batch) == 2:
                modalities, labels_or_ids = batch
                
                if isinstance(modalities, (tuple, list)) and len(modalities) == 3:
                    rgb_input, depth_input, ir_input = modalities
                else:
                    raise ValueError(f"意外的模态数据格式")
                
                if is_test_mode:
                    video_ids = labels_or_ids
                    labels = None
                    current_id = video_ids[0].item() if len(video_ids) > 0 else batch_idx
                else:
                    labels = labels_or_ids
                    video_ids = None
                    current_id = batch_idx
            else:
                raise ValueError(f"意外的batch格式")
            
            if batch_idx == 0:
                print(f"  - Video ID: {current_id}")
                print(f"  - 输入形状: RGB={rgb_input.shape}, Depth={depth_input.shape}, IR={ir_input.shape}")
                sys.stdout.flush()
            
            # TTA: 多次采样取平均
            if batch_idx == 0:
                print(f"  - 开始提取backbone特征 (TTA={tta_times}次)...")
                sys.stdout.flush()
            
            tta_features = []
            for tta_idx in range(tta_times):
                if batch_idx == 0 and tta_idx == 0:
                    print(f"    TTA {tta_idx+1}/{tta_times}: 传输数据到GPU...")
                    sys.stdout.flush()
                
                rgb_input_t = rgb_input.to(device, non_blocking=True)
                depth_input_t = depth_input.to(device, non_blocking=True)
                ir_input_t = ir_input.to(device, non_blocking=True)
                
                if batch_idx == 0 and tta_idx == 0:
                    print(f"    TTA {tta_idx+1}/{tta_times}: 提取backbone特征...")
                    sys.stdout.flush()
                
                # 提取backbone特征
                backbone_features = model(rgb_input_t, depth_input_t, ir_input_t)
                tta_features.append(backbone_features.cpu().numpy())
            
            backbone_features = np.mean(tta_features, axis=0)
            
            if batch_idx == 0:
                print(f"  - Backbone特征形状: {backbone_features.shape}")
                sys.stdout.flush()
            
            # 提取骨骼点特征（从RGB模态）- 优化：只处理关键帧
            if use_pose and pose_extractor is not None:
                if batch_idx == 0:
                    print(f"  - 开始提取骨骼点特征...")
                    sys.stdout.flush()
                
                # 将RGB输入转换为numpy格式（在CPU上）
                rgb_np = rgb_input.cpu().numpy()  # [B, C, T, H, W]
                # 转换为 [B, T, H, W, C] 格式
                rgb_np = rgb_np.transpose(0, 2, 3, 4, 1)  # [B, T, H, W, C]
                # 反归一化（ImageNet归一化）
                mean = np.array([0.485, 0.456, 0.406]).reshape(1, 1, 1, 1, 3)
                std = np.array([0.229, 0.224, 0.225]).reshape(1, 1, 1, 1, 3)
                rgb_np = rgb_np * std + mean
                rgb_np = np.clip(rgb_np, 0, 1)
                # 转换为uint8
                rgb_np = (rgb_np * 255).astype(np.uint8)
                # 提取骨骼点特征（优化：只处理中间几帧，减少计算量）
                # 只处理中间帧和首尾帧
                T = rgb_np.shape[1]
                if T > 4:
                    # 选择关键帧：首、中、尾
                    key_frames = [0, T//2, T-1]
                    rgb_key = rgb_np[:, key_frames, :, :, :]
                    if batch_idx == 0:
                        print(f"    - 选择关键帧: {key_frames} (总共{T}帧)")
                        sys.stdout.flush()
                    pose_features = pose_extractor.extract_pose_features(rgb_key)
                else:
                    pose_features = pose_extractor.extract_pose_features(rgb_np)
                
                if len(pose_features.shape) == 1:
                    pose_features = pose_features.reshape(1, -1)
                
                if batch_idx == 0:
                    print(f"  - 骨骼点特征形状: {pose_features.shape}")
                    sys.stdout.flush()
                
                # 拼接特征
                combined_features = np.concatenate([backbone_features, pose_features], axis=1)
            else:
                combined_features = backbone_features
            
            all_features.append(combined_features)
            if labels is not None:
                all_labels.extend(labels.numpy())
            if video_ids is not None:
                all_video_ids.extend(video_ids.numpy())
            
            # 更新进度信息
            batch_time = time.time() - batch_start
            elapsed = time.time() - start_time
            avg_time = elapsed / (batch_idx + 1)
            remaining = avg_time * (total_batches - batch_idx - 1)
            
            # 打印进度信息
            if (batch_idx + 1) % 10 == 0 or batch_idx == 0:
                print(f"  ✓ Batch {batch_idx+1}/{total_batches} 完成")
                print(f"    - 处理时间: {batch_time:.2f}秒")
                print(f"    - 已用时间: {elapsed/60:.1f}分钟")
                print(f"    - 预计剩余: {remaining/60:.1f}分钟")
                print(f"    - 平均速度: {avg_time:.2f}秒/batch")
                sys.stdout.flush()
            elif (batch_idx + 1) % 5 == 0:
                # 每5个batch简单打印一次
                print(f"  [Batch {batch_idx+1}/{total_batches}] 处理中... (已用 {elapsed/60:.1f}分钟, 剩余约 {remaining/60:.1f}分钟)")
                sys.stdout.flush()
            
            # 定期保存检查点
            if checkpoint_path and (batch_idx + 1) % checkpoint_interval == 0:
                checkpoint_data = {
                    'features': np.concatenate(all_features, axis=0) if all_features else None,
                    'labels': np.array(all_labels) if all_labels else None,
                    'video_ids': np.array(all_video_ids) if all_video_ids else None,
                    'batch_idx': batch_idx,
                    'total_batches': total_batches
                }
                cp_path = f"{checkpoint_path}_batch_{batch_idx+1}.pkl"
                joblib.dump(checkpoint_data, cp_path)
                # 使用tqdm.write确保不干扰进度条
                tqdm.write(f"\n✓ 检查点已保存: {cp_path}")
    
    all_features = np.concatenate(all_features, axis=0)
    
    if all_labels:
        all_labels = np.array(all_labels)
        return all_features, all_labels, None
    elif all_video_ids:
        return all_features, None, np.array(all_video_ids)
    else:
        return all_features, None, None


def main():
    parser = argparse.ArgumentParser(description='基于92.04分的增强版本（更强backbone + 骨骼点）')
    parser.add_argument('--data_root_train', type=str, default='MMAR/train_500')
    parser.add_argument('--video_list_train', type=str, default='MMAR/train_500/train_videofolder_500.txt')
    parser.add_argument('--data_root_test', type=str, default='MMAR/test_200')
    parser.add_argument('--video_list_test', type=str, default='MMAR/test_200/test_videofolder_200.txt')
    parser.add_argument('--backbone_names', type=str, default='convnext_large,regnet,efficientnet_v2',
                       help='backbone列表（用逗号分隔）')
    parser.add_argument('--use_pose', action='store_true', default=True,
                       help='是否使用骨骼点检测')
    parser.add_argument('--save_checkpoint', type=str, default=None,
                       help='检查点保存路径（每N个batch保存一次）')
    parser.add_argument('--checkpoint_interval', type=int, default=50,
                       help='检查点保存间隔（batch数）')
    parser.add_argument('--classifier', type=str, default='ensemble',
                       choices=['logistic', 'svm', 'rf', 'gbdt', 'ensemble'],
                       help='分类器类型')
    parser.add_argument('--ensemble_models', type=str, default='logistic,svm,rf',
                       help='集成学习的分类器列表（用逗号分隔）')
    parser.add_argument('--batch_size', type=int, default=1)
    parser.add_argument('--num_segments', type=int, default=8)
    parser.add_argument('--output', type=str, default='submission_pose_enhanced.csv')
    parser.add_argument('--gpu', type=int, default=0)
    parser.add_argument('--tta_times', type=int, default=5,
                       help='测试时增强次数（推荐5-10）')
    parser.add_argument('--tune_params', action='store_true',
                       help='自动调优超参数（会较慢，建议关闭）')
    parser.add_argument('--fast_tune', action='store_true',
                       help='快速调参（只搜索少量参数，推荐）')
    parser.add_argument('--skip_cv', action='store_true',
                       help='跳过交叉验证，使用平均权重（如果交叉验证卡住）')
    parser.add_argument('--use_pca', action='store_true',
                       help='使用PCA降维（可选）')
    parser.add_argument('--pca_components', type=int, default=2000,
                       help='PCA主成分数量')
    args = parser.parse_args()
    
    device = torch.device(f'cuda:{args.gpu}' if torch.cuda.is_available() else 'cpu')
    print(f'使用设备: {device}')
    if torch.cuda.is_available():
        print(f'GPU显存: {torch.cuda.get_device_properties(args.gpu).total_memory / 1024**3:.2f} GB')
    
    # 检查MediaPipe
    if args.use_pose and not MEDIAPIPE_AVAILABLE:
        print("警告: MediaPipe未安装，将跳过骨骼点检测")
        print("安装命令: pip install mediapipe")
        args.use_pose = False
    
    # 数据变换
    transform = transforms.Compose([
        transforms.ToPILImage(),
        transforms.Resize(256),
        transforms.CenterCrop(224),
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ])
    
    # 创建特征提取模型
    print("\n加载多backbone模型...")
    backbone_names = [n.strip() for n in args.backbone_names.split(',')]
    print(f"Backbone列表: {backbone_names}")
    print(f"使用骨骼点检测: {args.use_pose}")
    
    feature_extractor = MultiBackbonePoseExtractor(
        backbone_names=backbone_names,
        use_pose=args.use_pose
    ).to(device)
    feature_extractor.eval()
    print(f"总特征维度: {feature_extractor.feature_dim}")
    
    # 创建骨骼点提取器
    pose_extractor = feature_extractor.pose_extractor if args.use_pose else None
    
    # 提取训练集特征
    print("\n提取训练集特征...")
    train_dataset = MultiModalDataset(
        args.data_root_train, args.video_list_train,
        num_segments=args.num_segments,
        transform=transform,
        random_shift=False,
        test_mode=False
    )
    
    train_loader = DataLoader(
        train_dataset, batch_size=args.batch_size,
        shuffle=False, num_workers=2, pin_memory=True
    )
    
    # 设置检查点路径
    if args.save_checkpoint:
        checkpoint_path = args.save_checkpoint
    else:
        checkpoint_path = args.output.replace('.csv', '_checkpoint.pkl')
    
    train_features, train_labels, _ = extract_features_with_tta_and_pose(
        feature_extractor, pose_extractor, train_loader, device, 
        is_test_mode=False, tta_times=1, use_pose=args.use_pose,
        checkpoint_path=checkpoint_path, checkpoint_interval=args.checkpoint_interval
    )
    print(f"训练特征形状: {train_features.shape}")
    print(f"训练标签形状: {train_labels.shape}")
    
    # 特征预处理
    print("\n特征预处理...")
    if args.use_pca:
        print(f"使用PCA降维到 {args.pca_components} 维...")
        pca = PCA(n_components=args.pca_components, random_state=42)
        train_features = pca.fit_transform(train_features)
        print(f"PCA后特征形状: {train_features.shape}")
        print(f"解释方差比: {pca.explained_variance_ratio_.sum():.4f}")
    else:
        pca = None
    
    # 训练分类器
    print(f"\n训练{args.classifier}分类器...")
    
    trained_classifiers = None
    
    if args.classifier == 'ensemble':
        # 集成学习
        ensemble_names = [m.strip() for m in args.ensemble_models.split(',')]
        print(f"集成分类器: {ensemble_names}")
        
        classifiers = []
        for name in ensemble_names:
            if name == 'logistic':
                if args.tune_params or args.fast_tune:
                    if args.fast_tune:
                        param_grid = {'clf__C': [2.0, 3.0]}  # 只搜索2个参数
                        cv_folds = 3
                    else:
                        param_grid = {'clf__C': [1.0, 2.0, 3.0, 5.0]}
                        cv_folds = 5
                    base_clf = Pipeline([
                        ('scaler', RobustScaler()),
                        ('clf', LogisticRegression(max_iter=5000, solver='lbfgs', n_jobs=-1))
                    ])
                    cv = StratifiedKFold(n_splits=cv_folds, shuffle=True, random_state=42)
                    clf = GridSearchCV(base_clf, param_grid, cv=cv, scoring='accuracy', n_jobs=1, verbose=1)
                else:
                    clf = LogisticRegression(
                        max_iter=5000, 
                        solver='lbfgs', C=2.0, n_jobs=-1, random_state=42
                    )
            elif name == 'svm':
                if args.tune_params or args.fast_tune:
                    if args.fast_tune:
                        param_grid = {'clf__C': [3.0, 5.0], 'clf__gamma': ['scale']}  # 只搜索2个C值，1个gamma
                        cv_folds = 3
                    else:
                        param_grid = {'clf__C': [2.0, 3.0, 5.0, 10.0], 'clf__gamma': ['scale', 'auto']}
                        cv_folds = 5
                    base_clf = Pipeline([
                        ('scaler', RobustScaler()),
                        ('clf', SVC(kernel='rbf', probability=True, decision_function_shape='ovr', max_iter=10000))
                    ])
                    cv = StratifiedKFold(n_splits=cv_folds, shuffle=True, random_state=42)
                    clf = GridSearchCV(base_clf, param_grid, cv=cv, scoring='accuracy', n_jobs=1, verbose=1)
                else:
                    clf = SVC(
                        kernel='rbf', probability=True, C=3.0, gamma='scale', 
                        decision_function_shape='ovr', max_iter=10000, random_state=42
                    )
            elif name == 'rf':
                clf = RandomForestClassifier(
                    n_estimators=500, max_depth=20, 
                    n_jobs=-1, random_state=42
                )
            elif name == 'gbdt':
                clf = GradientBoostingClassifier(
                    n_estimators=200, max_depth=8, learning_rate=0.1,
                    subsample=0.8, max_features='sqrt', random_state=42, verbose=1
                )
            else:
                print(f"警告: 未知分类器 {name}，跳过")
                continue
            
            clf_pipeline = Pipeline([
                ('scaler', RobustScaler()),
                ('clf', clf)
            ])
            classifiers.append((name, clf_pipeline))
        
        if len(classifiers) == 0:
            raise ValueError("没有有效的分类器！")
        
        # 训练每个分类器
        print("\n训练各个分类器...")
        trained_classifiers = []
        for name, clf in classifiers:
            print(f"  训练 {name}...")
            import time
            start_time = time.time()
            clf.fit(train_features, train_labels)
            elapsed = time.time() - start_time
            print(f"  {name} 训练完成，耗时: {elapsed:.2f}秒")
            
            if args.tune_params and hasattr(clf, 'best_params_'):
                print(f"  最佳参数: {clf.best_params_}")
                print(f"  最佳分数: {clf.best_score_:.4f}")
            
            trained_classifiers.append((name, clf))
        
        # 交叉验证评估（使用较少折数，避免卡住）
        if args.skip_cv:
            print("\n跳过交叉验证，使用平均权重...")
            cv_scores = [1.0] * len(trained_classifiers)  # 使用平均权重
        else:
            print("\n评估集成分类器（交叉验证，使用3折以加快速度）...")
            print("=" * 60)
            print("提示: 如果卡住，可以按Ctrl+C跳过交叉验证，直接使用平均权重")
            import sys
            sys.stdout.flush()
            
            try:
                cv = StratifiedKFold(n_splits=3, shuffle=True, random_state=42)  # 减少到3折
                cv_scores = []
                for idx, (name, clf) in enumerate(trained_classifiers):
                    print(f"\n[{idx+1}/{len(trained_classifiers)}] 评估 {name}...")
                    sys.stdout.flush()
                    # 使用n_jobs=1避免死锁
                    scores = cross_val_score(clf, train_features, train_labels, cv=cv, scoring='accuracy', n_jobs=1, verbose=0)
                    cv_scores.append(scores.mean())
                    print(f"  ✓ {name} CV准确率: {scores.mean():.4f} (+/- {scores.std() * 2:.4f})")
                    sys.stdout.flush()
            except KeyboardInterrupt:
                print("\n\n⚠️  交叉验证被中断，使用平均权重...")
                cv_scores = [1.0] * len(trained_classifiers)  # 使用平均权重
            except Exception as e:
                print(f"\n\n⚠️  交叉验证出错: {e}")
                print("使用平均权重...")
                cv_scores = [1.0] * len(trained_classifiers)  # 使用平均权重
        
        # 使用加权集成（根据CV分数）
        cv_scores = np.array(cv_scores)
        weights = cv_scores / cv_scores.sum()
        print(f"\n集成权重: {dict(zip([n for n, _ in trained_classifiers], weights))}")
        
        # 创建集成预测函数
        def ensemble_predict_proba(X):
            if pca is not None:
                X = pca.transform(X)
            probas = []
            for (name, clf), weight in zip(trained_classifiers, weights):
                proba = clf.predict_proba(X)
                probas.append(proba * weight)
            return np.sum(probas, axis=0)
        
        class EnsembleClassifier:
            def __init__(self, predict_func, trained_classifiers, weights, pca):
                self.predict_proba = predict_func
                self.trained_classifiers = trained_classifiers
                self.weights = weights
                self.pca = pca
                self.is_fitted = True
            
            def fit(self, X, y):
                return self
        
        classifier = EnsembleClassifier(ensemble_predict_proba, trained_classifiers, weights, pca)
        classifier.fit(train_features, train_labels)
        print("集成分类器训练完成！")
    
    else:
        # 单个分类器
        if args.classifier == 'logistic':
            if args.tune_params or args.fast_tune:
                if args.fast_tune:
                    param_grid = {'clf__C': [2.0, 3.0]}
                    cv_folds = 3
                else:
                    param_grid = {'clf__C': [1.0, 2.0, 3.0, 5.0]}
                    cv_folds = 5
                base_clf = Pipeline([
                    ('scaler', RobustScaler()),
                    ('clf', LogisticRegression(max_iter=5000, solver='lbfgs', n_jobs=-1))
                ])
                cv = StratifiedKFold(n_splits=cv_folds, shuffle=True, random_state=42)
                classifier = GridSearchCV(base_clf, param_grid, cv=cv, scoring='accuracy', n_jobs=1, verbose=1)
            else:
                classifier = Pipeline([
                    ('scaler', RobustScaler()),
                    ('clf', LogisticRegression(
                        max_iter=5000, 
                        solver='lbfgs', C=2.0, n_jobs=-1
                    ))
                ])
        elif args.classifier == 'svm':
            if args.tune_params or args.fast_tune:
                if args.fast_tune:
                    param_grid = {'clf__C': [3.0, 5.0], 'clf__gamma': ['scale']}
                    cv_folds = 3
                else:
                    param_grid = {'clf__C': [2.0, 3.0, 5.0, 10.0], 'clf__gamma': ['scale', 'auto']}
                    cv_folds = 5
                base_clf = Pipeline([
                    ('scaler', RobustScaler()),
                    ('clf', SVC(kernel='rbf', probability=True, decision_function_shape='ovr', max_iter=10000))
                ])
                cv = StratifiedKFold(n_splits=cv_folds, shuffle=True, random_state=42)
                classifier = GridSearchCV(base_clf, param_grid, cv=cv, scoring='accuracy', n_jobs=1, verbose=1)
            else:
                classifier = Pipeline([
                    ('scaler', RobustScaler()),
                    ('clf', SVC(
                        kernel='rbf', probability=True, C=3.0, gamma='scale', 
                        decision_function_shape='ovr', max_iter=10000
                    ))
                ])
        elif args.classifier == 'rf':
            classifier = Pipeline([
                ('scaler', RobustScaler()),
                ('clf', RandomForestClassifier(
                    n_estimators=500, max_depth=20, 
                    n_jobs=-1, random_state=42
                ))
            ])
        else:  # gbdt
            classifier = Pipeline([
                ('scaler', RobustScaler()),
                ('clf', GradientBoostingClassifier(
                    n_estimators=200, max_depth=8, learning_rate=0.1,
                    subsample=0.8, max_features='sqrt', random_state=42, verbose=1
                ))
            ])
        
        print("开始训练分类器...")
        classifier.fit(train_features, train_labels)
        if args.tune_params and hasattr(classifier, 'best_params_'):
            print(f"最佳参数: {classifier.best_params_}")
            print(f"最佳分数: {classifier.best_score_:.4f}")
        print("分类器训练完成！")
    
    # 提取测试集特征（使用TTA）
    print(f"\n提取测试集特征（TTA={args.tta_times}）...")
    test_dataset = MultiModalDataset(
        args.data_root_test, args.video_list_test,
        num_segments=args.num_segments,
        transform=transform,
        random_shift=False,
        test_mode=True
    )
    
    test_loader = DataLoader(
        test_dataset, batch_size=args.batch_size,
        shuffle=False, num_workers=2, pin_memory=True
    )
    
    # 测试集检查点路径
    test_checkpoint_path = args.output.replace('.csv', '_test_checkpoint.pkl')
    
    test_features, _, test_video_ids = extract_features_with_tta_and_pose(
        feature_extractor, pose_extractor, test_loader, device, 
        is_test_mode=True, tta_times=args.tta_times, use_pose=args.use_pose,
        checkpoint_path=test_checkpoint_path, checkpoint_interval=args.checkpoint_interval
    )
    print(f"测试特征形状: {test_features.shape}")
    
    # 如果video_ids为None，从数据集获取
    if test_video_ids is None or len(test_video_ids) == 0:
        print("从数据集中获取video_ids...")
        test_video_ids = []
        for i in range(len(test_dataset)):
            _, video_id = test_dataset[i]
            test_video_ids.append(video_id)
        test_video_ids = np.array(test_video_ids)
    
    # 应用PCA（如果使用）
    if pca is not None:
        test_features = pca.transform(test_features)
    
    # 预测
    print("\n进行预测...")
    predictions_proba = classifier.predict_proba(test_features)
    
    # 后处理：轻微的温度缩放
    predictions_proba = predictions_proba ** 0.9
    predictions_proba = predictions_proba / predictions_proba.sum(axis=1, keepdims=True)
    
    # 获取Top-5预测
    top5_indices = np.argsort(predictions_proba, axis=1)[:, -5:][:, ::-1]
    
    # 按video_id排序
    sorted_indices = np.argsort(test_video_ids)
    top5_indices = top5_indices[sorted_indices]
    test_video_ids = test_video_ids[sorted_indices]
    
    # 生成提交文件
    prediction_strings = []
    for pred in top5_indices:
        pred_str = ' '.join([str(int(cls)) for cls in pred])
        prediction_strings.append(pred_str)
    
    df = pd.DataFrame({
        'video_id': test_video_ids,
        'prediction': prediction_strings
    })
    
    df = df.sort_values('video_id').reset_index(drop=True)
    df.to_csv(args.output, index=False, header=True)
    
    print(f"\n提交文件已保存到: {args.output}")
    print(f"文件包含 {len(df)} 行数据")
    print("\n前10行预览:")
    print(df.head(10).to_string(index=False))
    
    # 保存分类器和PCA
    try:
        classifier_path = args.output.replace('.csv', '_classifier.pkl')
        joblib.dump(classifier, classifier_path)
        print(f"\n分类器已保存到: {classifier_path}")
        
        if pca is not None:
            pca_path = args.output.replace('.csv', '_pca.pkl')
            joblib.dump(pca, pca_path)
            print(f"PCA已保存到: {pca_path}")
    except Exception as e:
        print(f"\n警告: 保存模型失败: {e}")


if __name__ == '__main__':
    import cv2
    main()

