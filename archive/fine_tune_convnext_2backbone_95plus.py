...
class TwoBackbonePoseExtractor(nn.Module):
    """2个backbone + 骨骼点特征提取器（关键：使用L2归一化）"""
    def __init__(self, backbone_names=['convnext_large', 'regnet'], use_pose=True):
        super(TwoBackbonePoseExtractor, self).__init__()
...
def main():
    parser = argparse.ArgumentParser(description='基于92.54分的2-backbone版本（目标95+分）')
    ...
    parser.add_argument('--output', type=str, default='submission_2backbone_95plus.csv')
    parser.add_argument('--gpu', type=int, default=0)
    parser.add_argument('--backbone_names', type=str, default='convnext_large,regnet',
                       help='backbone列表（用逗号分隔，推荐2个）')
    ...
    feature_extractor = TwoBackbonePoseExtractor(
