...
class ConvNeXtLargeExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            features_max = features.max(dim=1)[0]
            features_std = features.std(dim=1)
            # 拼接多种时序聚合方式
            features = torch.cat([features_mean, features_max, features_std], dim=1)
...
class MultiModalConvNeXtExtractor(nn.Module):
    ...
    def __init__(self):
        ...
        # 每个模态：1536 * 3 (mean, max, std) = 4608
        # 三个模态：4608 * 3 = 13824
        self.feature_dim = self.rgb_extractor.feature_dim * 3 * 3
...
def extract_features_with_tta(model, dataloader, device, is_test_mode=False, tta_times=10):
    """提取特征，支持强TTA"""
    model.eval()
    all_features = []
    all_labels = []
...
def main():
    parser = argparse.ArgumentParser(description='ConvNeXt-Large微调到95+分')
    ...
    parser.add_argument('--output', type=str, default='submission_convnext_95plus.csv')
    ...
    print(f"特征维度: {feature_extractor.feature_dim} (包含mean/max/std时序聚合)")
    ...
    print("\n特征预处理...")
    if args.use_pca:
        # PCA组件数不能超过样本数和特征数的最小值
        n_samples, n_features = train_features.shape
        max_components = min(n_samples, n_features, args.pca_components)
        if max_components < args.pca_components:
            print(f"警告: PCA组件数从 {args.pca_components} 调整为 {max_components} (样本数限制)")
        print(f"使用PCA降维到 {max_components} 维...")
    ...
    def ensemble_predict_proba(X):
        # 注意：X应该已经是PCA转换后的（如果使用了PCA）
        # 因为测试集特征在传入前已经经过PCA转换
        probas = []
