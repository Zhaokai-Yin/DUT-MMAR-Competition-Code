...
def main():
    parser = argparse.ArgumentParser(description='终极零样本优化版本（目标95+分，不训练）')
    ...
    parser.add_argument('--output', type=str, default='submission_95plus_ultimate.csv')
    ...
    if args.classifier == 'ensemble':
        ...
        if args.use_calibration:
            print("\n应用概率校准...")
            clf_svm = CalibratedClassifierCV(clf_svm, method='isotonic', cv=3)
            clf_lr = CalibratedClassifierCV(clf_lr, method='isotonic', cv=3)
            clf_rf = CalibratedClassifierCV(clf_rf, method='isotonic', cv=3)
            clf_svm.fit(train_features, train_labels)
            clf_lr.fit(train_features, train_labels)
            clf_rf.fit(train_features, train_labels)
            print("  ✓ 概率校准完成")
