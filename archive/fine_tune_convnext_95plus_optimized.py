...
class ConvNeXtLargeExtractor(nn.Module):
    """ConvNeXt Large特征提取器（支持多时序聚合）"""
    def __init__(self, use_multi_temporal=True):
        ...
        # 如果使用多时序聚合，特征维度变为3倍
        if use_multi_temporal:
            self.feature_dim = self.feature_dim * 3  # mean, max, std
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            if self.use_multi_temporal:
                ...
                feat_max = features.max(dim=1)[0]
                feat_std = features.std(dim=1)
                features = torch.cat([feat_mean, feat_max, feat_std], dim=1)
            else:
                # 只使用mean（与92.54分一致）
...
class MultiModalConvNeXtPoseExtractor(nn.Module):
    ...
    def __init__(self, use_pose=True, use_multi_temporal=True):
        super(MultiModalConvNeXtPoseExtractor, self).__init__()
        self.rgb_extractor = ConvNeXtLargeExtractor(use_multi_temporal=use_multi_temporal)
        self.depth_extractor = ConvNeXtLargeExtractor(use_multi_temporal=use_multi_temporal)
        self.ir_extractor = ConvNeXtLargeExtractor(use_multi_temporal=use_multi_temporal)
        self.convnext_dim = self.rgb_extractor.feature_dim * 3
...
def main():
    parser = argparse.ArgumentParser(description='基于92.54分的优化版本（目标95+分）')
    ...
    parser.add_argument('--output', type=str, default='submission_95plus_optimized.csv')
    parser.add_argument('--gpu', type=int, default=0)
    parser.add_argument('--tta_times', type=int, default=15,
                       help='测试时增强次数（推荐15-20次）')
    parser.add_argument('--use_pose', action='store_true', default=True,
                       help='是否使用骨骼点检测')
    parser.add_argument('--use_multi_temporal', action='store_true', default=True,
                       help='是否使用多时序聚合（mean, max, std）')
    parser.add_argument('--temperature', type=float, default=0.92,
                       help='温度缩放参数（默认0.92，更平滑）')
    ...
    # 创建特征提取模型
    print("\n加载ConvNeXt Large模型...")
    print(f"使用多时序聚合: {args.use_multi_temporal}")
    feature_extractor = MultiModalConvNeXtPoseExtractor(
        use_pose=args.use_pose, 
        use_multi_temporal=args.use_multi_temporal
    ).to(device)
    ...
    if args.classifier == 'ensemble':
        ...
        clf_svm = Pipeline([
            ...
            ('clf', SVC(
                kernel='rbf', probability=True, C=4.0, gamma='scale',  # 从3.0增加到4.0
        ...
        # 2. Logistic Regression（优化C值）
        print("  训练 Logistic Regression...")
        clf_lr = Pipeline([
            ('scaler', StandardScaler()),
            ('clf', LogisticRegression(
                max_iter=5000, solver='lbfgs', C=2.5, n_jobs=-1, random_state=42  # 从2.0增加到2.5
            ))
        ])
        clf_lr.fit(train_features, train_labels)
        print("  ✓ Logistic Regression 完成")
        
        # 3. Random Forest（优化参数）
        print("  训练 Random Forest...")
        clf_rf = Pipeline([
            ('scaler', StandardScaler()),
            ('clf', RandomForestClassifier(
                n_estimators=600, max_depth=25, n_jobs=-1, random_state=42  # 从500/20增加到600/25
            ))
        ])
