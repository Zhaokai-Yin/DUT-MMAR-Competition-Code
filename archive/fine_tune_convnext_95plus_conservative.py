...
def main():
    parser = argparse.ArgumentParser(description='基于92.54分的保守优化版本（目标95+分）')
    ...
    parser.add_argument('--output', type=str, default='submission_95plus_conservative.csv')
    parser.add_argument('--gpu', type=int, default=0)
    parser.add_argument('--tta_times', type=int, default=20,
                       help='测试时增强次数（推荐15-25次，只改这个参数）')
    ...
    if args.classifier == 'ensemble':
        ...
        # 2. Logistic Regression（与92.54分完全一致）
    ...
    # 提取测试集特征（使用强TTA，这是唯一的优化）
    print(f"\n提取测试集特征（TTA={args.tta_times}，这是唯一的优化）...")
