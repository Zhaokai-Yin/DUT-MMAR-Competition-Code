"""
基于93.53分的进一步优化版本（目标95+分，不训练）
策略：保持ConvNeXt Large，尝试不同的优化组合
"""
import os
import argparse
import torch
...
def main():
    parser = argparse.ArgumentParser(description='基于93.53分的进一步优化版本（目标95+分）')
    ...
    parser.add_argument('--output', type=str, default='submission_95plus_v2.csv')
    parser.add_argument('--gpu', type=int, default=0)
    parser.add_argument('--tta_times', type=int, default=15,
                       help='测试时增强次数（从10增加到15）')
    ...
    parser.add_argument('--feature_selection_k', type=int, default=3200,
                       help='特征选择保留的特征数（从3000增加到3200）')
