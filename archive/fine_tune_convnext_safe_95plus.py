...
def main():
    parser = argparse.ArgumentParser(description='ConvNeXt-Large安全优化到95+分（基于92.04分）')
    ...
    parser.add_argument('--output', type=str, default='submission_convnext_safe_95plus.csv')
    ...
    # 创建特征提取模型（与92.04分配置一致）
    print("\n加载ConvNeXt Large模型...")
    feature_extractor = MultiModalConvNeXtExtractor().to(device)
    feature_extractor.eval()
    print(f"特征维度: {feature_extractor.feature_dim} (与92.04分配置一致)")
