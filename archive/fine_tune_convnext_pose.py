...
class PoseExtractor:
    ...
    def extract_pose_features(self, images):
        ...
        从图像序列中提取骨骼点特征
        Args:
            images: numpy array [B, T, H, W, C] 或 [T, H, W, C]，uint8格式，RGB通道
        Returns:
        ...
        if len(images.shape) == 5:  # [B, T, H, W, C]
            ...
            if img.dtype != np.uint8:
                if img.max() <= 1.0:
                    ...
                    img = img.astype(np.uint8)
            return self._extract_single_frame(img)
...
def main():
    parser = argparse.ArgumentParser(description='ConvNeXt-Large + 骨骼点检测（目标95+分）')
    ...
    parser.add_argument('--output', type=str, default='submission_convnext_pose.csv')
    ...
    if args.use_pose and not MEDIAPIPE_AVAILABLE:
        ...
        print("安装命令: pip install mediapipe>=0.10.0")
        args.use_pose = False
