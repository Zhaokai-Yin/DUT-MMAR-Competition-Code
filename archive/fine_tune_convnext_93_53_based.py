"""
基于93.53分的保守优化版本（目标95+分，不训练）
策略：保持93.53分的成功配置，只做微小调整
"""
import os
import argparse
import torch
...
class PoseExtractor:
    ...
    def __init__(self, use_enhanced_features=True):
        ...
        self.pose = self.mp_pose.Pose(
            ...
            model_complexity=2,  # 最高复杂度
            enable_segmentation=False,
            min_detection_confidence=0.5,
            min_tracking_confidence=0.5
        )
        
        # 基础特征：33个关键点 × 4个值（x, y, z, visibility）
        base_dim = 33 * 4  # 132维
        ...
        if use_enhanced_features:
            ...
            # 距离特征：6个关键距离
            distance_dim = 6
            # 统计特征：关键点的mean, std（简化版本，只计算2个统计量）
            stats_dim = 33 * 2  # 每个关键点的2个统计量（mean, std）
            
            enhanced_dim = angle_dim + distance_dim + stats_dim
            self.feature_dim = base_dim + enhanced_dim
        else:
        ...
        try:
            ...
            if results.pose_landmarks:
                ...
                for landmark in landmarks:
                    base_features.extend([landmark.x, landmark.y, landmark.z, landmark.visibility])
                ...
                # 1. 计算关键关节角度
                for joint_triple in self.important_joints[:4]:  # 前4个是角度
                    if len(joint_triple) == 3:
                        p1_idx, p2_idx, p3_idx = joint_triple
                        ...
                        enhanced_features.append(angle)
                
                # 2. 计算关键距离
                for joint_pair in self.important_joints[4:]:  # 后2个是距离
                    if len(joint_pair) == 2:
                        p1_idx, p2_idx = joint_pair
                ...
                # 3. 统计特征（关键点的统计量）- 简化版本，只计算mean和std
                landmark_coords = np.array([[lm.x, lm.y, lm.z, lm.visibility] for lm in landmarks])
                stats = [
                    np.mean(landmark_coords, axis=0),  # mean
                    np.std(landmark_coords, axis=0),   # std
                ]
                enhanced_features.extend(np.concatenate(stats))
...
class MultiModalConvNeXtPoseExtractor(nn.Module):
    ...
    def __init__(self, use_pose=True, use_enhanced_pose=False):
        ...
        if use_pose and MEDIAPIPE_AVAILABLE:
            # 根据参数决定是否使用增强特征
            self.pose_extractor = PoseExtractor(use_enhanced_features=use_enhanced_pose)
...
def main():
    parser = argparse.ArgumentParser(description='基于93.53分的保守优化版本（目标95+分）')
    ...
    parser.add_argument('--output', type=str, default='submission_93_53_based.csv')
    ...
    parser.add_argument('--temperature', type=float, default=0.88,
                       help='温度缩放参数（93.53分使用0.88）')
    parser.add_argument('--use_enhanced_pose', action='store_true', default=False,
                       help='是否使用增强的骨骼点特征（默认False，因为可能很慢）')
    ...
    # 创建特征提取模型
    print("\n加载ConvNeXt Large模型...")
    feature_extractor = MultiModalConvNeXtPoseExtractor(
        use_pose=args.use_pose, 
        use_enhanced_pose=args.use_enhanced_pose
    ).to(device)
    feature_extractor.eval()
    print(f"特征维度: {feature_extractor.feature_dim}")
    if args.use_pose:
        if args.use_enhanced_pose:
            print("使用增强的骨骼点特征（276维）")
        else:
            print("使用基础骨骼点特征（132维）")
    ...
    if args.use_feature_selection:
        print(f"\n特征选择（保留前{args.feature_selection_k}个特征，与93.53分一致）...")
    ...
    # 训练分类器（使用93.53分的参数）
    print(f"\n训练{args.classifier}分类器（使用93.53分的参数）...")
    ...
    if args.classifier == 'ensemble':
        ...
        # 2. Logistic Regression（93.53分：C=2.5）
        ...
        # 3. Random Forest（93.53分：n_estimators=600, max_depth=25）
    ...
    # 后处理：温度缩放（93.53分使用0.88）
    print(f"使用温度缩放: {args.temperature}（与93.53分一致）")
